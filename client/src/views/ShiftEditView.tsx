...
export default function ShiftEditView({ isActive, cal, currentMonthLabel, setCm, users, shifts, setShifts, todayIso, toast }: ShiftEditViewProps) {
  ...
  useEffect(() => {
    if (isActive) setEditShifts(shifts);
  }, [isActive, shifts]);
  ...
  const visibleUsers = useMemo(() => {
    ...
    const extras = selectedTabShifts
      ...
      .filter((user): user is User => Boolean(user) && !base.some((baseUser) => baseUser.id === user.id));
    return [...base, ...extras];
